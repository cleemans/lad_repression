...
```
damlam=/home/t.v.schaik/mydata/proj/3D_nucleus/results/ts170821_GCF4577_K562_p13_LMNB1/results/tracks_counts/LMNB1-gatc.bw
damonly=/home/t.v.schaik/mydata/proj/3D_nucleus/results/ts170821_GCF4577_K562_p13_LMNB1/results/tracks_counts/Dam16-gatc.bw
bwtool summary -header \
               ...
               <(awk -vOFS='\t' '{
                 ...
                 $damlam /dev/stdout  | gzip -c > raw_data/body_damlam.txt.gz
...
bwtool summary -header \
            ...
            <(awk -vOFS='\t' '{
              ...
              $damonly /dev/stdout | gzip -c > raw_data/body_damonly.txt.gz
```

```{r}
dam_body = read.table('../raw_data/body_damonly.txt.gz', row.names=4, stringsAsFactors=F,
                      col.names=c('chrom', 'start', 'end', 'name', 'score',
                                  'strand', 'size', 'num_data', 'min', 'max',
                                  'mean', 'sum'))
lam_body = read.table('../raw_data/body_damlam.txt.gz', row.names=4, stringsAsFactors=F,
                      col.names=c('chrom', 'start', 'end', 'name', 'score',
                                  'strand', 'size', 'num_data', 'min', 'max',
                                  'mean', 'sum'))

damlam = log2((lam_body[P$name, 'sum'] + 1) / (dam_body[P$name, 'sum'] + 1))

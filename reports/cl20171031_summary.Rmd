...
ggplot(data, aes(x=log2(tssr_POL2A + body_POL2A),
...
group_mean <- function(signal_data, P, class_name='class_GROcap',
    ...
    pos_vec = (start / step) : (end / step - 1) + .5
    data = data.frame(class=p_matched[, class_name], signal_data[p_matched$name, ])
    ...
    class_vec = unique(data$class)
    i_vec = 1:length(class) * 3
...
proseq_sense_norm$mean = proseq_mean$mean / max_vec[as.character(proseq_mean$class)]
proseq_sense_norm$max = proseq_mean$max / max_vec[as.character(proseq_mean$class)]
proseq_sense_norm$min = proseq_mean$min / max_vec[as.character(proseq_mean$class)]
...
proseq_antisense_norm$mean = proseq_antisense_mean$mean /
                                max_vec[as.character(proseq_mean$class)]
...
ggplot(data, aes(x=pos, y=mean, linetype=direction)) +
    ...
    facet_wrap(~class) +
    theme_bw() +
    coord_cartesian(ylim=c(-0.25,0.5), xlim=c(-5000,20000))
...
melt_data = melt(mean_data, measure.vars=c('escaper', 'iLAD'))
ggplot(melt_data, aes(x=pos, y=value / 200, color=variable,
                          linetype=direction)) +
...
mean_data$iLAD = mean_data$iLAD / max(mean_data$iLAD)
mean_data$escaper = mean_data$escaper / max(mean_data$escaper)
melt_data = melt(mean_data, measure.vars=c('escaper', 'iLAD'))
ggplot(melt_data, aes(x=pos, y=value, color=variable,
                          linetype=direction)) +
    geom_vline(xintercept=0, linetype='dotdash', size=0.2) +
    facet_wrap(~variable) +
    geom_line() +

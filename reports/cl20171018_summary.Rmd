...
bwtool summary -header \
            ...
            ~/mydata/data/tracks/hg19/POL2A_K562_fcoc_ENCFF552VTT.bigWig \
            /dev/stdout | gzip -c > raw_data/tssr_POL2A_K562_ENCFF552VTT.txt.gz &
...
bwtool summary -header \
             ...
             <(awk -vOFS='\t' '{
                ...
                print $1, start, end, $4, 0, $6}' raw_data/tss_hg19.bed) \
             ~/mydata/data/tracks/hg19/POL2A_K562_fcoc_ENCFF552VTT.bigWig \
             /dev/stdout | gzip -c > raw_data/body_POL2A_K562_ENCFF552VTT.txt.gz &
...
bwtool summary -header \
               ...
               <(awk -vOFS='\t' '{
                 ...
                 print $1, start, end, $4, 0, $6}' raw_data/tss_hg19.bed) \
             ~/mydata/data/tracks/hg19/POL2AS2_K562_ENCFF354XDX.bigWig \
             /dev/stdout | gzip -c > raw_data/tssr_POL2AS2_K562.txt.gz &
...
bwtool summary -header \
               ...
               <(awk -vOFS='\t' '{
                  ...
                  print $1, start, end, $4, 0, $6}' raw_data/tss_hg19.bed) \
               ~/mydata/data/tracks/hg19/POL2AS2_K562_ENCFF354XDX.bigWig \
               /dev/stdout | gzip -c > raw_data/body_POL2AS2_K562.txt.gz &
...
bwtool summary -header \
              ...
              <(awk -vOFS='\t' '{
                ...
                print $1, start, end, $4, 0, $6}' raw_data/tss_hg19.bed) \
            ~/mydata/data/tracks/hg19/K562_POL2AS5_fcoc_ENCFF827GEM.bigWig \
            /dev/stdout | gzip -c > raw_data/tssr_POL2AS5_K562.txt.gz &
...
bwtool summary -header \
              ...
              <(awk -vOFS='\t' '{
                 ...
                 print $1, start, end, $4, 0, $6}' raw_data/tss_hg19.bed) \
              ~/mydata/data/tracks/hg19/K562_POL2AS5_fcoc_ENCFF827GEM.bigWig \
              /dev/stdout | gzip -c > raw_data/body_POL2AS5_K562.txt.gz &
```
...
P$tssr_POL2A = pol2_tssr[P$name, 'mean']
P$body_POL2A = pol2_body[P$name, 'mean']
...
ggplot(p_pol2, aes(x=class_GROcap, y=log10(value), color=class_GROcap)) +
    ...
    xlab('promoter class') +
    stat_summary(color = "red", geom = "crossbar", width=0.3) +
    facet_wrap('variable')
...
```{r figure4a.3, echo=FALSE}


pol2_tssr = read.table('../raw_data/tssr_POL2A_K562_ENCFF552VTT.txt.gz', row.names=4,
                       col.names=c('chrom', 'start', 'end', 'name', 'score',
                                   'strand', 'size', 'num_data', 'min', 'max',
                                   'mean', 'sum'), stringsAsFactors=F)

pol2_body = read.table('../raw_data/body_POL2A_K562_ENCFF552VTT.txt.gz', row.names=4, skip=1,
                       col.names=c('chrom', 'start', 'end', 'name', 'score',
                                  'strand', 'size', 'num_data', 'min', 'max',
                                  'mean', 'sum'), stringsAsFactors=F)
pol2_tssr$mean = pol2_tssr$sum / pol2_tssr$num_data
...
```{r figure4a.4, fig.width=10, fig.height=10, echo=FALSE}
p_matched = matchSet(P[P$class_GROcap%in%c('escaper', 'iLAD'), ], 'class_GROcap',
                     'escaper', 'GROcap_K562')
p_pol2 = melt(p_matched[,c('class_GROcap', 'tssr_POL2A', 'body_POL2A')])
p_pol2$variable = factor(p_pol2$variable)
...
```{r figure4a.5, echo=FALSE}


pol2s2_tssr = read.table('../raw_data/tssr_POL2AS2_K562.txt.gz', row.names=4,
                       col.names=c('chrom', 'start', 'end', 'name', 'score',
                                   'strand', 'size', 'num_data', 'min', 'max',
                                   'mean', 'sum'), stringsAsFactors=F)

pol2s2_body = read.table('../raw_data/body_POL2AS2_K562.txt.gz', row.names=4, skip=1,
                       col.names=c('chrom', 'start', 'end', 'name', 'score',
                                  'strand', 'size', 'num_data', 'min', 'max',
                                  'mean', 'sum'), stringsAsFactors=F)
pol2s2_tssr$mean = pol2s2_tssr$sum / pol2s2_tssr$num_data
pol2s2_body$mean = pol2s2_body$sum / pol2s2_body$num_data

P$tssr_POL2AS2 = pol2s2_tssr[P$name, 'mean']
P$body_POL2AS2 = pol2s2_body[P$name, 'mean']

```


```{r figure4a.6, fig.width=10, fig.height=10, echo=FALSE}
p_matched = matchSet(P[P$class_GROcap%in%c('escaper', 'iLAD'), ], 'class_GROcap',
                     'escaper', 'GROcap_K562')
p_pol2s2 = melt(p_matched[,c('class_GROcap', 'tssr_POL2AS2', 'body_POL2AS2')])
p_pol2s2$variable = factor(p_pol2s2$variable)
levels(p_pol2s2) = c("TSS region (-50bp:+300bp from TSS)",
                   "Gene body (+300bp:3000bp from TSS)")
ggplot(p_pol2s2, aes(x=class_GROcap, y=log10(value), color=class_GROcap)) +
    geom_violin() +
    geom_point(position=position_jitter(width=0.1), alpha=0.5, size=0.5) +
    scale_color_manual(values=COL_class) +
    xlab('promoter class') +
...
ggplot(p_matched, aes(x=log2(tssr_POL2AS2 + body_POL2AS2),
                      y=log10(tssr_POL2AS2 / body_POL2AS2),
                      color=class_GROcap)) +
    geom_point(alpha=0.5, size=0.5) +
    geom_smooth() +
    scale_color_manual(values=COL_class)
...
```{r figure4a.7, echo=FALSE}


pol2s5_tssr = read.table('../raw_data/tssr_POL2AS5_K562.txt.gz', row.names=4,
                       col.names=c('chrom', 'start', 'end', 'name', 'score',
                                   'strand', 'size', 'num_data', 'min', 'max',
                                   'mean', 'sum'), stringsAsFactors=F)

pol2s5_body = read.table('../raw_data/body_POL2AS5_K562.txt.gz', row.names=4, skip=1,
                       col.names=c('chrom', 'start', 'end', 'name', 'score',
                                  'strand', 'size', 'num_data', 'min', 'max',
                                  'mean', 'sum'), stringsAsFactors=F)
pol2s5_tssr$mean = pol2s5_tssr$sum / pol2s5_tssr$num_data
pol2s5_body$mean = pol2s5_body$sum / pol2s5_body$num_data

P$tssr_POL2AS5 = pol2s5_tssr[P$name, 'mean']
P$body_POL2AS5 = pol2s5_body[P$name, 'mean']

```


```{r figure4a.8, fig.width=10, fig.height=10, echo=FALSE}
p_matched = matchSet(P[P$class_GROcap%in%c('escaper', 'iLAD'), ], 'class_GROcap',
                     'escaper', 'GROcap_K562')
p_pol2s5 = melt(p_matched[,c('class_GROcap', 'tssr_POL2AS5', 'body_POL2AS5')])
p_pol2s5$variable = factor(p_pol2s5$variable)
levels(p_pol2s5) = c("TSS region (-50bp:+300bp from TSS)",
                   "Gene body (+300bp:3000bp from TSS)")
ggplot(p_pol2s5, aes(x=class_GROcap, y=log10(value), color=class_GROcap)) +
    geom_violin() +
    geom_point(position=position_jitter(width=0.1), alpha=0.5, size=0.5) +
    scale_color_manual(values=COL_class) +
    xlab('promoter class') +
...
ggplot(p_matched, aes(x=log2(tssr_POL2AS5 + body_POL2AS5),
                      y=log10(tssr_POL2AS5 / body_POL2AS5),
                      color=class_GROcap)) +
    geom_point(alpha=0.5, size=0.5) +
    geom_smooth() +
    scale_color_manual(values=COL_class)
...
```{r figure4a.9, fig.width=10, fig.height=10, echo=FALSE}
p_matched = matchSet(P[P$class_GROcap%in%c('escaper', 'iLAD'), ], 'class_GROcap',
                     'escaper', 'GROcap_K562')
p_pol2s = melt(p_matched[,c('class_GROcap', 'tssr_POL2AS5', 'body_POL2AS2')])
p_pol2s$variable = factor(p_pol2s$variable)
levels(p_pol2s) = c("TSS region (-50bp:+300bp from TSS)",
                   "Gene body (+300bp:3000bp from TSS)")
ggplot(p_pol2s, aes(x=class_GROcap, y=log10(value), color=class_GROcap)) +
    geom_violin() +
    geom_point(position=position_jitter(width=0.1), alpha=0.5, size=0.5) +
    scale_color_manual(values=COL_class) +
    xlab('promoter class') +
...
ggplot(p_matched, aes(x=log2(tssr_POL2AS5 + body_POL2AS2),
                      y=log10(tssr_POL2AS5 / body_POL2AS2),
                      color=class_GROcap)) +
    geom_point(alpha=0.5, size=0.5) +
    geom_smooth() +
    scale_color_manual(values=COL_class)

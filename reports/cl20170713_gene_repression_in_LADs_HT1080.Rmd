...
```bash
bwtool summary -header <(awk -vOFS='\t' '{print $1, $2 - 500, $3 + 500, $4, $5, $6}' workspace/tss.bed) raw_data/DNase-seq/K562_rep1_ENCFF001BSF.bigWig /dev/stdout > results/tss_DNAse_K562_rep1.txt

bwtool summary -header <(awk -vOFS='\t' '{print $1, $2 - 500, $3 + 500, $4, $5, $6}' workspace/tss.bed) raw_data/DNase-seq/K562_rep2_ENCFF001DPE.bigWig /dev/stdout > results/tss_DNAse_K562_rep2.txt

bwtool summary -header <(awk -vOFS='\t' '{print $1, $2 - 500, $3 + 500, $4, $5, $6}' workspace/tss.bed) raw_data/DNase-seq/HT1080_rep1_ENCFF930ZWL.bigWig /dev/stdout > results/tss_DNAse_HT1080_rep1.txt

bwtool summary -header <(awk -vOFS='\t' '{print $1, $2 - 500, $3 + 500, $4, $5, $6}' workspace/tss.bed) raw_data/DNase-seq/HT1080_rep2_ENCFF760VXZ.bigWig /dev/stdout > results/tss_DNAse_HT1080_rep2.txt
...
for (col in c('SuRE_K562', 'SuRE_HT1080', 'GROcap_K562', 'CAGE_K562', 'CAGE_HT1080', 'K562_RAMPAGE', 'HT1080_RAMPAGE', 'DNAse_K562', 'DNAse_HT1080')){
...
```{r}
classify_3d <- function(sure, exp_y, exp_z, lrs_y, lrs_z, lad, exp_cut_y, exp_cut_z){
    INACT<- sure< -0.3 & lad & exp_y < exp_cut_y  & exp_z < exp_cut_z #inactive
    NREP<- sure> 0 & lrs_y > -0.5 & lrs_z > -0.5 & lad & exp_y > exp_cut_y  & exp_z > exp_cut_z #not repressed
    REP<- sure> 0.3 & lrs_y < -0.75 & lrs_z < -0.75 & lad  & exp_y < exp_cut_y  & exp_z < exp_cut_z #repressed
    Pcnts<-c(length(which(REP)), length(which(NREP)), length(which(INACT)))
    names(Pcnts)<-c("repressed", "escaper", "inactive")
    BND <- lad & !INACT & !NREP & !REP
    class = rep(NA, length(sure))
...
P$class_K562_3D = classify_3d(P$SuRE_K562,
                              ...
                              P$LRS_K562, P$LRS_K562_DNAse, P$LAD_K562, 0.25, 1)

P$class_HT1080_3D = classify_3d(P$SuRE_HT1080,
                                P$CAGE_HT1080, P$DNAse_HT1080,
                                P$LRS_HT1080, P$LRS_HT1080_DNAse, P$LAD_HT1080, 0.5, 1.75)

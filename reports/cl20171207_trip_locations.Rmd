...
fwd_file_list = list.files('../../trip/cl20171206_TRIP_K562_evsr/mapping',
                           pattern='1.table', full.names=T)
names(fwd_file_list) = gsub('.*mapping/(.*).1.table', '\\1', fwd_file_list)


rev_file_list = list.files('../../trip/cl20171206_TRIP_K562_evsr/mapping',
...
for (prom in c('6','11')){
    for (bio in unique(map_info$bio_rep)){
        ...
        map_gr = get_data_table(rev_data, lad_hmm_gr, lmnb1_5kb)
...
all_lad_repressed = all_gr[all_gr$LAD=='AD' & all_gr$unique, ]

all_lad_repressed$exp_state = NA
all_lad_repressed$exp_state[all_lad_repressed$expression < -9] = "OFF"
all_lad_repressed$exp_state[all_lad_repressed$expression > -1] = "ON"
...
all_lad_repressed$nearest_Pradeepa = mcols(distanceToNearest(all_lad_repressed,
                                                       H3K27_Pradeepa_peaks))$distance

all_lad_repressed$nearest_H3K4me1 = mcols(distanceToNearest(all_lad_repressed,
                                                            H3K4me1_Pradeepa_peaks))$distance

all_lad_repressed$nearest_schmidl = mcols(distanceToNearest(all_lad_repressed,
                                                       H3K27_schmidl_peaks))$distance

all_lad_repressed$nearest_H3K9ac = mcols(distanceToNearest(all_lad_repressed,
                                                           H3K9ac_peaks))$distance


all_lad_repressed$nearest_ATAC = mcols(distanceToNearest(all_lad_repressed,
                                                         ATAC_peaks))$distance

nearest_peak = apply()
data = as.data.frame(mcols(all_lad_repressed))
...
ggplot(data, aes(x=log10(nearest_Pradeepa), y=expression)) +
    geom_point() +
    ggtitle('H3K27ac Pradeepa et al.')

ggplot(data, aes(x=log10(nearest_schmidl), y=expression)) +
    geom_point() +
    ggtitle('H3K27ac Schmidl et al.')

ggplot(data, aes(x=log10(nearest_H3K4me1), y=expression)) +
    geom_point() +
    ggtitle('H3K4me1 Pradeepa et al.')

ggplot(data, aes(x=log10(nearest_H3K9ac), y=expression)) +
    geom_point() +
    ggtitle('H3K9ac ENCODE')

ggplot(data, aes(x=log10(nearest_ATAC), y=expression, alpha=lamin)) +
    geom_point() +
    ggtitle('ATAC-seq')

ggplot(data, aes(x=log10(nearest_peak), y=expression, alpha=lamin)) +
    geom_point() +
    ggtitle('nearest active histone mark (H3K27ac, H3K4me1, H3K9ac)')
```
...
for (class in c('iLAD', 'escaper', 'inactive', 'repressed')){
    ...
    data = as.data.frame(mcols(all_lad_repressed))
    data$nearest = Inf
    near = distanceToNearest(all_lad_repressed,
                             tss_gr[tss_gr$name==class])
    data[queryHits(near), 'nearest'] = mcols(near)$distance
    print(ggplot(data, aes(x=log10(nearest), y=expression)) +
              geom_point() +
